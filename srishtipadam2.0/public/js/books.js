/**
 * Srishtipadham Website - Books Page Engine
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const applyFiltersAndRender = () => {
    // 1. Filtering
    let filteredBooks = SrishtipadhamData.books.filter(book => {
...
buyBtn.addEventListener("click", () => {
    // 1. Set the official Srishtipadam WhatsApp number.
    // Make sure to include the country code (91 for India) without the '+' symbol or spaces.
    const srishtipadamPhone = "919446771277"; // REPLACE with your actual number

    // 2. Create the message template
    // Note: This assumes you have the 'author' variable available in this scope, just like 'title'
    let message = `Hi, I wish to order "${title}" written by ${author}.`
    ;

    // 3. (Optional) Provide the Malayalam translation if the site is in Malayalam mode
    if (state.lang === "ml") {
        message = `നമസ്കാരം, ${author} എഴുതിയ "${title}" എന്ന പുസ്തകം ഓർഡർ ചെയ്യാൻ ഞാൻ ആഗ്രഹിക്കുന്നു.`;
    }

    // 4. Encode the text so it can be passed safely in a URL
    const encodedMessage = encodeURIComponent(message);

    // 5. Construct the final WhatsApp URL
    const whatsappURL = `https://wa.me/${srishtipadamPhone}?text=${encodedMessage}`;

    // 6. Open WhatsApp in a new browser tab/app
    window.open(whatsappURL, '_blank');

    // 7. Close the modal after clicking
    closeModal();
});
  };
