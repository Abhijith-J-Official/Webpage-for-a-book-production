/**
 * Srishtipadham Website - Core Engine
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const state = {
    lang: localStorage.getItem("srishtipadham_lang") || "en",
    theme: localStorage.getItem("srishtipadham_theme") || "light"
  };
  ...
  document.querySelectorAll(".theme-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      localStorage.setItem("srishtipadham_theme", state.theme);
      initTheme();
    });
  });
  ...
  const translatePage = () => {
    const dictionary = SrishtipadhamData.translations[state.lang];
  ...
  document.querySelectorAll(".lang-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      localStorage.setItem("srishtipadham_lang", state.lang);
      initLanguage();
  ...
  const renderTeamGrid = () => {
    ...
    SrishtipadhamData.team.forEach(member => {
  ...
  const renderUpdatesGrid = () => {
    ...
    const filtered = SrishtipadhamData.updates.filter(item => {
  ...
  const openUpdateModal = (item) => {
    ...
    const dict = SrishtipadhamData.translations[state.lang];
  ...
  const setupJoinUsModal = () => {
    ...
    const openModal = () => {
      modal.classList.add("active");
      document.body.style.overflow = "hidden"; // Fixed: ensure it targets document.body
    ...
    const closeModal = () => {
      modal.classList.remove("active");
      document.body.style.overflow = ""; // Fixed: ensure it targets document.body
    };
    ...
    form.addEventListener("submit", (e) => {
      ...
      const userMessage = form.querySelector("#join-message").value.trim(); // Renamed to avoid conflicts
      ...
      if (!userMessage || userMessage.length < 10) isValid = false;
      ...
      // --- NEW WHATSAPP REDIRECTION LOGIC ---
      
      // 1. Set the contact number (include country code 91, no '+' or spaces)
      const srishtipadamPhone = "919446771277"; 

      // 2. Construct the structured multi-line message
      const whatsappText = `Hi, my name is ${name}, I wish to join Srishtipadham.
...
${userMessage}`;
      ...
      // 4. Create the final URL and open it
      const whatsappURL = `https://wa.me/${srishtipadamPhone}?text=${encodedMessage}`;
      window.open(whatsappURL, '_blank');

      // --- END WHATSAPP LOGIC ---

      // Show success response in the UI behind the new tab
